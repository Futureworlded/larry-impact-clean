...
Here is how every part of Larry Impact works and what to do in each section. No tech speak — just day-to-day use.
...
Larry Impact is a platform where supporters buy products and part of every sale goes to an animal rescue organization. A typical order goes like this:
...
The system also handles refunds and disputes. If a customer is refunded after a payout already went out, the rescue's next payout gets adjusted automatically.
...
Where a rescue applies to join.

**What the rescue does:**
1.  Go to `/apply/`.
2.  Fill out the application form.
3.  Use a real rescue email, or a fake test address like `someone@example.com` if you are just testing.
4.  Submit the form.
...
- Once approved, the rescue gets login info by email and can sign in.

### Rescue login

Where approved rescues sign in.
...
After signing in, rescues see `/dashboard/`. The dashboard has tabs:

#### Overview
- Welcomes the rescue and shows a quick account summary.
- If the rescue page is not ready yet, it may say "Your profile is being set up. Check back shortly."

#### My profile
- The rescue can update its profile picture, description, location, contact info, social media links, and representative.
- This shows up on the public rescue page.

#### My earnings
- Shows total sales, rescue earnings, and any pending payout.
- Rescues can see which orders have been recorded and their status: pending, ready for payout, approved, or paid.

#### Setup / My account
- Use this to connect a payment account so the rescue can get direct deposits.
...
Each approved rescue gets its own public page. These pages live under `/rescues/`.
...
- The system creates one page per approved rescue.

---

## Admin areas

All admin tools live under **Larry Impact** in the website admin panel.

### Rescues

The full list of rescues.
...
**What you do:**
- Review the list.
- Pending rescues show up here only after they are approved on the Applications page.
- If a rescue's profile is missing, click the rescue row or use **Rescue Pages** to sync its public page.

### Applications

Where you review new rescue applications.
...
4.  To approve, click **Approve**. The system will:
    ...
    - Email the rescue its login info.
...
**Testing tip:**
- When testing the approval flow, use a fake identity like `Test Rescue / someone@example.com`. Do not approve real applications unless you plan to onboard that rescue.

### Split Dashboard

Shows how revenue is split between the rescue, Larry Impact, and product costs.
...
- If a split looks off, go to **Split Configurator** to fix it.

### Split Configurator

Set how much of each product sale goes to the rescue and how much Larry Impact keeps.

**Important concept:**
- Splits are saved as a history of versions. When you change a split, the system records the new version with a date.
- Old orders keep the split that was active when the order happened. New orders use the latest version.
...
Where you manage money owed to rescues.

**Important concept:**
The payout workflow has four stages:
1.  **Pending** — The order has been recorded, but no payout batch has been created.
2.  **Ready for payout** — A batch has been prepared, but no money has been sent.
3.  **Approved** — The admin approved the batch and the transfer is in progress.
4.  **Paid / Completed** — The money reached the rescue.
5.  **Archived** — The batch is complete and stored for records.
...
4.  Each batch appears in the **Payout batches** table as "Ready for payout."
...
- Use the **Filter** dropdown to show all orders, pending, ready, approved, or paid.
...
- Hand this to your bookkeeper or accountant.

### Settings

Where platform-wide defaults are set.
...
- **Currency code and symbol** — The currency used for reports and payouts, like `USD` and `$`.
- **Payment credentials** — The connection to the payment company. Only the person managing the payment account should change these.
...
- If a customer is refunded before the payout, the order is marked refunded and the rescue is not paid for that order.
- If a customer is refunded after the payout already went out, the system records a negative entry on the rescue's next payout.

### Chargeback or dispute

- When a payment dispute comes in, the system marks the order as disputed.
- Stop the related payout if possible and adjust future payouts until the dispute is resolved.

---

## Important notes

- **Do not approve or change real rescue data unless you mean it.** Use fake info like `someone@example.com` when testing the application flow.
- **Double-check a rescue's payout details before approving a batch.** If a rescue has not connected a payment account, the transfer will fail.
- **Payouts are not automatic until the payment connection is set up.** Each batch still needs manual approval.
- **Keep the ledger export for your books.** The spreadsheet export in Reports is the official record of sales, payouts, refunds, and adjustments.
- **Have issues? Call me at 925-871-9044.** If you need to change payment account settings, currencies, or the minimum payout threshold, call me first — these affect real money.